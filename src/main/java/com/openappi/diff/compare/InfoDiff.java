...
public class InfoDiff {
	...
	public static Info diff(Info oldInfo, Info newInfo) {
		...
		if (ComparisonUtils.isDiff(oldInfo.toString(), newInfo.toString())) {
			...
			if (Objects.nonNull(oldContact)) {
				if (Objects.nonNull(newContact)) {
					if (ComparisonUtils.isDiff(oldContact.toString(), newContact.toString())) {
						...
						if (ComparisonUtils.isDiff(oldContact.getEmail(), newContact.getName())) {
			...
			if (Objects.nonNull(oldLicense)) {
				if (Objects.nonNull(newLicense)) {
					if (ComparisonUtils.isDiff(oldLicense.toString(), newLicense.toString())) {
